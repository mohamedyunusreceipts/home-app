...
import { SectionCard } from '@/components/vault/section-card'
...
export default async function VaultPage() {
  ...
  for (const d of documents) {
    if (d.expiry_date) expiring.push({ label: d.name, detail: `Document · ${d.kind}`, date: d.expiry_date })
  }
  for (const w of warranties) {
    if (w.expiry_date) expiring.push({ label: w.item, detail: 'Warranty', date: w.expiry_date })
  }
  for (const v of vehicles) {
    if (v.insurance_expiry) expiring.push({ label: v.label, detail: 'Vehicle insurance', date: v.insurance_expiry })
    if (v.license_expiry) expiring.push({ label: v.label, detail: 'Vehicle licence', date: v.license_expiry })
    if (v.service_due_date) expiring.push({ label: v.label, detail: 'Service due', date: v.service_due_date })
  }
  for (const vd of vehicleDocs) {
    if (vd.expiry_date) expiring.push({ label: vehicleLabel(vd.vehicle_id), detail: `Car document · ${vd.kind}`, date: vd.expiry_date })
  }
    ...
    <main className="min-h-screen p-8">
      <div className="mx-auto max-w-3xl space-y-6">
        <header className="space-y-1">
          <h1 className="font-serif text-3xl text-terracotta-700">Vault</h1>
          <p className="text-sage-600">
            Documents, emergency contacts, car papers, warranties and gift ideas —
            all in one safe place.
          </p>
        </header>

        {/* Upcoming expiries — surfaces anything due within 60 days. */}
        <SectionCard
          title="Coming up"
          description="Documents, warranties and vehicle dates expiring soon."
        >
          {upcoming.length === 0 ? (
            <p className="text-sm text-sage-600">
              Nothing expiring in the next 60 days. You&apos;re all set.
            </p>
          ) : (
            <ul className="space-y-2">
              {upcoming.map((e, i) => (
                <li
                  ...
                  className="flex items-center justify-between gap-3 border-b border-sage-100 pb-2 last:border-0 last:pb-0"
                >
                  <div>
                    <p className="font-medium text-sage-900">{e.label}</p>
                    <p className="text-xs text-sage-600">{e.detail} · {formatDate(e.date)}</p>
                  </div>
        ...
        {/* Documents vault + IDs/passports (same table, kind discriminator). */}
        <SectionCard
          title="Documents vault"
          description="IDs, passports, contracts and other important papers."
          action={<span className="text-sm text-sage-500">{documents.length}</span>}
        >
          {documents.length === 0 ? (
            ...
            <ul className="space-y-2">
              {documents.map((d) => (
                <li key={d.id} className="flex items-center justify-between gap-3 border-b border-sage-100 pb-2 last:border-0 last:pb-0">
                  <div>
                    <p className="font-medium text-sage-900">{d.name}</p>
        ...
        <SectionCard
          ...
          description="Who to call when it matters."
          action={<span className="text-sm text-sage-500">{contacts.length}</span>}
        >
          {contacts.length === 0 ? (
            ...
            <ul className="space-y-2">
              {contacts.map((c) => (
                <li key={c.id} className="flex items-center justify-between gap-3 border-b border-sage-100 pb-2 last:border-0 last:pb-0">
                  <div>
        ...
        <SectionCard
          title="Vehicles"
          description="Cars, with insurance, licence and service dates."
          action={<span className="text-sm text-sage-500">{vehicles.length}</span>}
        >
          {vehicles.length === 0 ? (
            ...
            <ul className="space-y-2">
              {vehicles.map((v) => (
                <li key={v.id} className="border-b border-sage-100 pb-2 last:border-0 last:pb-0">
                  <p className="font-medium text-sage-900">
                    {v.label}
        ...
        <SectionCard
          title="Car documents"
          description="Insurance certs, licence discs and service records."
          action={<span className="text-sm text-sage-500">{vehicleDocs.length}</span>}
        >
          {vehicleDocs.length === 0 ? (
            ...
            <ul className="space-y-2">
              {vehicleDocs.map((vd) => (
                <li key={vd.id} className="flex items-center justify-between gap-3 border-b border-sage-100 pb-2 last:border-0 last:pb-0">
                  <div>
                    <p className="font-medium text-sage-900">{vehicleLabel(vd.vehicle_id)}</p>
                    <p className="text-xs text-sage-600">
                      {vd.kind}
        ...
        <SectionCard
          ...
          description="Keep proof of purchase and expiry handy."
          action={<span className="text-sm text-sage-500">{warranties.length}</span>}
        >
          {warranties.length === 0 ? (
            ...
            <ul className="space-y-2">
              {warranties.map((w) => (
                <li key={w.id} className="flex items-center justify-between gap-3 border-b border-sage-100 pb-2 last:border-0 last:pb-0">
                  <div>
                    <p className="font-medium text-sage-900">{w.item}</p>
                    <p className="text-xs text-sage-600">
        ...
        {/* Gift ideas — recipient never sees ideas for themselves (enforced by RLS). */}
        <SectionCard
          title="Gift ideas"
          description="Ideas for your partner and others — hidden from the recipient."
          action={<span className="text-sm text-sage-500">{gifts.length}</span>}
        >
          {gifts.length === 0 ? (
            ...
            <ul className="space-y-2">
              {gifts.map((g) => (
                <li key={g.id} className="flex items-center justify-between gap-3 border-b border-sage-100 pb-2 last:border-0 last:pb-0">
                  <div>
                    <p className="font-medium text-sage-900">
                      {g.url ? (
